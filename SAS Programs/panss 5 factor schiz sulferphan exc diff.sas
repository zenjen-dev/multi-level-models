...
run;
  ...
  ****** Model1 generalized mixed - unstructured panss diff -measure model  **********;

proc glimmix data=unidata;
  class SubNo2 groupcd time ;
  /*distribution/link (transform)/DF syntax indicated in below line */
  model pnsexcdf = PANSSExcit5FactW2 groupcd time  time*groupcd / dist=poisson link=log ddf=146,146,146,146 solution;
  random intercept / subject=SubNo2 type=un;
  random _residual_;
  lsmeans time*groupcd/diffs;
    ods output diffs=drugdiffs lsmeans=druglsmeans;
  title 'repeated panss excitement 5factor diff  unstructured model with baseline cov'; 
run;

  ****** Model1 generalized mixed - unstructured panss diff -measure model DF method  **********;

proc glimmix data=unidata;
  class SubNo2 groupcd time ;
  /*distribution/link (transform)/DF syntax indicated in below line */
  model pnsexcdf = PANSSExcit5FactW2 groupcd time  time*groupcd / dist=poisson link=log ddfm=betwithin solution;
  random intercept / subject=SubNo2 type=un;
  random _residual_;
  lsmeans time*groupcd/diffs;
    ods output diffs=drugdiffs lsmeans=druglsmeans;
