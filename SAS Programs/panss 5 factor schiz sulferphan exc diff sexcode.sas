...
proc glimmix data=unidata;
  ...
  model pnsexcdf=PANSSExcit5FactW2 groupcd  time  SEXCODE time*groupcd SEXCODE*groupcd  
        SEXCODE*time*groupcd / dist=poisson link=log solution;
  random intercept / subject=SubNo2 type=un;
  random _residual_;
  lsmeans time*groupcd/diffs;
    ods output diffs=drugdiffs lsmeans=druglsmeans;
  title 'repeated panss excitement 5factor unstructured model with baseline  cov and sex code'; 
run;
