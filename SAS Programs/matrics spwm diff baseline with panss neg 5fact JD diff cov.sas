...
data multidata ;
  set schizchina;
  keep SubNo2 groupcd PANNSNegFactJDaddW12W2DIFF PANNSNegFactJDaddW24W2DIFF W2MCCBWorking MATRICSPATWMW12W2DIFF MATRICSPATWMW24W2DIFF ;  
  run;
...
proc mixed data=unidata method=ml ;
  ...
  model spwmdiff = W2MCCBWorking PANNSNegFactJDaddW12W2DIFF  groupcd time  time*groupcd/solution;
  ...
  title 'repeated measures  matrics  spwm diff baseline with panss neg 5 fact JD w12w2 diff cov '; 
run;

****** Model1 -- Unstructured matrics spwm total  diff,  **********;
proc mixed data=unidata method=ml ;
  class  SubNo2 groupcd time;
  model spwmdiff = W2MCCBWorking PANNSNegFactJDaddW24W2DIFF  groupcd time  time*groupcd/solution;
  repeated time /subject=SubNo2 type=un r rcorr;
  lsmeans time*groupcd/diffs;
  ods output diffs=groupcdiffs lsmeans=groupcdlsmeans;
  title 'repeated measures  matrics  spwm diff baseline with panss neg 5 fact JD w24w2 diff cov '; 
run;