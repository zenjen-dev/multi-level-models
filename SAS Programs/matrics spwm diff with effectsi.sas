PROC IMPORT OUT= WORK.schizchina 
            DATAFILE= "C:\Users\jenarriaza\SAS Files\Data\sulforaphane matrics spwm diff  r1.sav"
...
data multidata ;
  set schizchina;
  keep SubNo2 groupcd YearsILLmonths SEXCODE MATRICSPATWMW12W2DIFF MATRICSPATWMW24W2DIFF W2MCCBWorking;  
  run;
...
TITLE1 "repeated measures matrics spatial working mem unstructured model with effectsize";
...
run;
%mend;
%effectsize_rep(file=unidata,y=spwmdiff,class= SubNo2 groupcd time, fixed =W2MCCBWorking groupcd time  time*groupcd);
/* above line specifies file and model components/interaction in mixed model */
proc print data=sasuser.spwmdiff; run;
